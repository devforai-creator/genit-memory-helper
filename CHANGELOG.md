...
## Unreleased
- 새 UI 패널/모달 체계를 기능 플래그(`gmh_flag_newUI`)와 킬스위치(`gmh_kill`)로 감싸 베타 채널에서만 활성화되도록 분리
- 레거시 패널과 프라이버시 프롬프트를 유지하면서도 베타 모드에서 상태기계·접근성·키보드 숏컷을 실험할 수 있도록 런타임 토글 추가
- Stable/Beta 이중 채널 배포 방식을 도입하고 README에 설치/푸시 전략, 킬스위치 사용법을 문서화
- 패널을 플로팅 버튼(FAB) 기반 도킹 모드로 개편하고 ESC/Alt+M 토글, 자동 접기(10초), 패널 외부 클릭 감지를 추가해 모바일/협소 화면에서도 가려짐 없이 동작하도록 개선
