...
- **genit.ai 스크롤 컨테이너 감지 수정**: 전체 페이지 스크롤 레이아웃 대응
  - GMH 패널이 스크롤 컨테이너로 잘못 감지되는 문제 수정
  - `scrollingElement` 폴백 추가로 DOM 구조 변경에 유연하게 대응
- **중복 청크 저장 방지**: "GMH에 담기" 클릭 시 이미 저장된 ordinalRange와 동일한 청크는 스킵
  - 기존 요약/Facts 보존
  - 상태 메시지에 저장/스킵 개수 표시
- **메타 요약 중복 저장 방지**: 동일한 chunkRange의 메타 요약이 이미 있으면 기존 ID 재사용 (업데이트)
  - 저장/업데이트 상태 메시지 구분
