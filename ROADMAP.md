...
> **프로젝트 상태: 활성 개발**
> v2.4.0 (2025-01-30) 기준, 사용자 피드백 기반 기능 개선 및 기술 부채 정리 진행 중.
> 의미 검색/임베딩 기능은 보류 상태.

**버전**: 2.4.0 (2025-01-30 갱신)
**현재 상태**: 멀티 플랫폼 지원 완료 (genit.ai + babechat.ai)
**다음 목표**: 기술 부채 정리 + 안정화
**기반**: TypeScript + 블록화 인프라 + 멀티 플랫폼 어댑터
...
v2.3.0 [Multi-Platform] ✅      (완료: 2025-01-29) - babechat.ai 지원
    ↓
v2.4.0 [UX] ✅                  (완료: 2025-01-30) - FAB 위치 설정 ← 현재
    ↓
v2.5.0 [Tech Debt] 🔄           계획 중 - 기술 부채 정리
    ↓
v3.0.0 [Semantic Search] ⏸️     보류 - 의미 검색, 임베딩
```
...
| 항목 | 설명 | 우선순위 |
|------|------|----------|
| **Major 의존성 업그레이드** | vite 5→7, vitest 2→4, jsdom 24→27 (보안 취약점 해결) | 🔴 HIGH |
| **babechat 어댑터 테스트** | XHR 인터셉션, API 수집 테스트 커버리지 | 🟡 MEDIUM |
| **privacy-gate 테스트** | 사용자 확인 모달 테스트 | 🟡 MEDIUM |
| **share 워크플로우 테스트** | Export 오케스트레이션 테스트 | 🟡 MEDIUM |
| **코드 중복 리팩토링** | DOM 헬퍼, 이벤트 바인딩 유틸리티 추출 | 🟢 LOW |

### 보류된 마일스톤 (v3.0.0+)

| 버전 | 목표 | 핵심 기능 | 상태 |
|------|------|----------|------|
| **v3.0.0** | 의미 검색 | BGE-M3 임베딩, 검색 UI, 맥락 재구성 | ⏸️ 보류 |
| **v3.1.0** | 성능 최적화 | 자동 로더 캐싱, 검색 캐싱 | ⏸️ 보류 |
| **v3.2.0+** | 고급 기능 | Ollama 연동, 시간 필터, 태깅 | ⏸️ 보류 |
