...
> **프로젝트 상태: Dual Memory v3.1.0 릴리즈**
> v3.1.0 (2025-12-01) 기준, Meta Summary (계층적 요약) 완료.
> 다음 목표: v3.2.0 UX Polish (프롬프트 커스터마이징, 백업/복원)

**버전**: 3.1.0-roadmap (2025-12-01 갱신)
**현재 상태**: Meta Summary 완료 (10개 청크 → 1개 메타 요약, 계층적 복사)
**다음 목표**: v3.2.0 UX Polish
...
v2.5.0 [Tech Debt] ✅           (완료: 2025-11-30) - 취약점 0개, 테스트 +48개
    ↓
v3.0.0 [Dual Memory] ✅         (완료: 2025-12-01) - 요약 + Facts 추출
    ↓
v3.0.1 [Bug Fix] ✅             (완료: 2025-12-01) - 저장 로드 타이밍 수정
    ↓
v3.1.0 [Meta Summary] ✅        (완료: 2025-12-01) - 메타 요약 (10개 청크 → 1개 통합)
    ↓
v3.2.0 [UX Polish] 📋           계획됨 - 프롬프트 커스터마이징, 백업/복원 ← 다음 목표
    ↓
v3.3.0+ [Advanced] ⏸️           보류 - Ollama 연동, 외부 API
    ↓
...
| **v3.0.0** | Dual Memory Core (청크화, 요약/Facts, IndexedDB) | ✅ 완료 | 2025-12-01 |
| **v3.0.1** | 버그 수정 (저장 로드 타이밍) | ✅ 완료 | 2025-12-01 |
| **v3.1.0** | Meta Summary (10개 청크 → 1개 메타, 계층적 복사) | ✅ 완료 | 2025-12-01 |
...
### 완료/예정 마일스톤 (Dual Memory)

| 버전 | 목표 | 핵심 기능 | 상태 |
|------|------|----------|------|
| **v3.0.0** | Dual Memory Core | 10개 청크화, 요약/Facts 프롬프트, IndexedDB 저장 | ✅ 완료 (2025-12-01) |
| **v3.0.1** | Bug Fix | 저장 로드 타이밍 수정 (waitForStorage) | ✅ 완료 (2025-12-01) |
| **v3.1.0** | Meta Summary | 10개 청크 요약 → 1개 메타 요약, 계층적 복사 | ✅ 완료 (2025-12-01) |
| **v3.2.0** | UX Polish | 진행 상태 표시, 백업/복원, 프롬프트 커스터마이징 | 📋 계획됨 |
| **v3.3.0+** | Advanced | Ollama 연동, 외부 API (BYOK) | ⏸️ 보류 |
...
- [x] 10개 메시지 청크화 로직 (`memory-chunker.ts`)
- [x] 청크 뷰어 UI (`dual-memory-controls.ts`)
- [x] 요약/Facts 프롬프트 템플릿 (`memory-prompts.ts`)
- [x] 프롬프트 + 청크 복사 기능
- [x] 결과 붙여넣기 UI (Phase 2)
- [x] 청크별 요약/Facts 저장 (IndexedDB - `block-storage.ts` 확장)
- [x] 통합 메모리 뷰 (유저노트용 복사 섹션)
- [x] 유저노트용 포맷 내보내기 (전체 요약/Facts/통합 복사)
- [x] 기존 UI와 통합 (`panel-modern.ts` Dual Memory 섹션)
- [x] 테스트 추가 (`memory-chunker.spec.ts` 18개)

### v3.0.0 성공 기준

- [x] 100개 메시지 → 10개 청크 정상 생성
- [x] 프롬프트 복사 → 외부 LLM → 결과 붙여넣기 워크플로우 동작
- [x] 유저노트용 최종 출력 생성
- [x] 기존 내보내기 기능과 공존

---

## 🔄 v3.1.0 - Meta Summary (메타 요약)

**목표**: 10개 청크 요약을 1개 메타 요약으로 통합하는 계층적 요약 시스템
**상태**: 📋 계획됨
**예상 작업량**: 8-12시간
...
```
문제: 100+ 메시지 대화
├─ 10개 청크 × 200자 요약 = 2000자 (유저노트 한계!)
├─ 200개 메시지 = 20개 청크 × 200자 = 4000자 (초과!)
└─ 계속 늘어나면 관리 불가

해결: 계층적 압축
├─ 청크 1-10 요약 (10×200자) → 메타 요약 1 (500자)
├─ 청크 11-20 요약 (10×200자) → 메타 요약 2 (500자)
└─ 청크 21-25 요약 → 개별 유지 (아직 10개 안 됨)

전체 요약 복사 시:
├─ 메타 요약 1 (500자) ← 청크 1-10 개별 요약 제외
├─ 메타 요약 2 (500자) ← 청크 11-20 개별 요약 제외
└─ 청크 21-25 개별 요약 (1000자)
= 총 2000자 (유저노트에 딱 맞음!)
```
...
| 기능 | 설명 |
|------|------|
| **메타 요약 생성** | 10개 청크 요약이 모이면 "메타 요약 생성" 버튼 활성화 |
| **메타 요약 프롬프트** | 10개 요약을 합쳐서 500자 내외로 압축하는 프롬프트 |
| **메타 요약 결과 입력** | LLM 결과 붙여넣기 → IndexedDB 저장 |
| **계층적 복사** | 전체 요약 복사 시 메타 요약된 청크는 제외 |
| **메타 요약 배지** | 메타 요약에 포함된 청크에 "📦 메타1" 표시 |

### 데이터 구조

```typescript
interface MetaSummary {
  id: string;                    // meta-1, meta-2, ...
  sessionUrl: string;
  chunkIds: string[];            // 포함된 청크 ID (10개)
  chunkRange: [number, number];  // 청크 인덱스 범위 (0-9, 10-19, ...)
  summary: string;               // 메타 요약 결과
  timestamp: number;
}

// block-storage.ts에 추가
interface BlockStorageController {
  // 기존 메서드...
  saveMeta(meta: MetaSummary): Promise<void>;
  getMetaBySession(sessionUrl: string): Promise<MetaSummary[]>;
...
│ └─────────────────────────────────┘ │
│                                     │
│ [메타 요약 생성] ← 청크 21-30 준비됨 │
├─────────────────────────────────────┤
│ 📝 청크 목록                        │
│ ┌─────────────────────────────────┐ │
│ │ #1 (1-10) 📦메타1  [요약][Facts]│ │
│ │ #2 (11-20) 📦메타1 [요약][Facts]│ │
│ │ ...                              │ │
│ │ #11 (101-110)      [요약][Facts]│ │← 아직 메타 요약 안 됨
│ └─────────────────────────────────┘ │
├─────────────────────────────────────┤
│ 📋 유저노트 복사                    │
│ [전체 요약] [전체 Facts] [통합]     │
...
**Phase 1: 데이터 구조**
- [ ] `MetaSummary` 타입 정의 (types/index.ts)
- [ ] block-storage.ts에 메타 요약 저장/조회 메서드 추가
- [ ] IndexedDB 스키마 확장 (meta-summaries store)

**Phase 2: 메타 요약 생성**
- [ ] 메타 요약 프롬프트 템플릿 (memory-prompts.ts)
- [ ] "메타 요약 생성" 버튼 (10개 청크 요약 완료 시 활성화)
- [ ] 메타 요약 결과 입력 UI
- [ ] 메타 요약 IndexedDB 저장
...
- [ ] 20개 청크 → 메타 요약 2개 정상 생성
- [ ] 전체 요약 복사 시 메타 요약만 포함 (개별 청크 제외)
- [ ] 유저노트 2000자 이내로 출력 가능
- [ ] 기존 워크플로우(청크 요약/Facts)와 공존

---

## 🎨 v3.2.0 - UX Polish (사용성 개선)
...
| 기능 | 설명 |
|------|------|
| **진행 상태** | 몇 개 청크 완료됐는지 시각적 표시 (프로그레스 바) |
| **가져오기/내보내기** | JSON 백업/복원 |
| **프롬프트 커스터마이징** | 사용자 정의 요약/Facts/메타 프롬프트 |
| **설정 영구 저장** | localStorage에 프롬프트 설정 저장 |
...
## 🔧 v3.3.0+ - Advanced (고급 기능)

**목표**: 자동화 옵션 제공
**상태**: ⏸️ 보류 (v3.2.0 완료 후 재검토)
**예상 작업량**: 15-25시간
...
| **자동 메타 요약** | Ollama/API로 메타 요약 자동 생성 | 중 |
...
| 마일스톤 | 핵심 목표 | 예상 시간 | 상태 |
|---------|----------|----------|------|
| **v3.0.0** | Dual Memory Core (청크화, 요약/Facts) | 20-30h | ✅ 완료 |
| **v3.0.1** | 버그 수정 (저장 로드 타이밍) | 1h | ✅ 완료 |
| **v3.1.0** | Meta Summary (계층적 요약) | 8-12h | 📋 계획됨 |
| **v3.2.0** | UX Polish (진행 상태, 백업/복원) | 10-15h | 📋 계획됨 |
| **v3.3.0+** | Advanced (Ollama, 외부 API) | 15-25h | ⏸️ 보류 |

**총 예상**: 35-55시간 (v3.1.0 ~ v3.3.0)
...
### v3.1.0 성공 기준 (Meta Summary)
- [ ] 20개 청크 → 메타 요약 2개 정상 생성
- [ ] 전체 요약 복사 시 메타 요약만 포함 (개별 청크 제외)
- [ ] 유저노트 2000자 이내로 출력 가능
- [ ] 기존 워크플로우(청크 요약/Facts)와 공존

### v3.2.0 성공 기준 (UX Polish)
- [ ] 청크 완료 상태 시각적 표시
- [ ] JSON 백업/복원 동작
- [ ] 커스텀 프롬프트 저장/불러오기
...
**최종 업데이트**: 2025-12-01
**작성**: Claude Code
**다음 리뷰**: v3.1.0 (Meta Summary) 개발 착수 시
