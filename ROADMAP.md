...
**버전**: 2.0 (2025-10-24 갱신)
**현재 상태**: v2.0.1 릴리스 완료 ✅
**다음 목표**: v2.1.0 Infrastructure (실시간 인덱서 + 블록화)
**기반**: TypeScript 전환 완료 + Embedding PoC 검증 완료
...
v2.0.1 (현재) ✅
    ↓
v2.1.0 [Infrastructure]      ← 3-4주 (20-30h)
    ↓
v2.2.0 [Semantic Search]     ← 3-4주 (25-35h)
    ↓
v2.3.0 [Performance]         ← 2-3주 (15-20h)
    ↓
v2.4.0+ [Power User Features]
```

**총 예상 시간**: 60-85시간 (v2.1.0 ~ v2.3.0)
...
| 버전 | 목표 | 핵심 기능 | 우선순위 |
|------|------|----------|----------|
| **v2.1.0** | 인프라 구축 | 실시간 인덱서, 블록화, IndexedDB | 🔴 HIGH |
| **v2.2.0** | 의미 검색 알파 | BGE-M3, 검색 UI, 맥락 재구성 | 🔴 HIGH |
| **v2.3.0** | 성능 최적화 | 자동 로더 캐싱, 매직 넘버 상수화 | 🟡 MEDIUM |
| **v2.4.0+** | 고급 기능 | Ollama 연동, 시간 필터, 태깅 | 🟢 LOW |
