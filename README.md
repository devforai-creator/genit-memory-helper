...
- **턴 범위 내보내기 (실험적)**: Export 섹션의 `턴 범위` 입력에 시작/끝 **플레이어 턴**(가장 최근 턴부터 1, 2, … 순서)을 적어 원하는 구간만 파일로 저장할 수 있습니다. `시작지정`/`끝지정` 버튼을 누르면 최근 클릭한 플레이어 턴이 북마크 큐(최대 5개)에 저장되고, 드롭다운에서 과거 책갈피를 다시 선택할 수 있습니다. 다만 대화 맨 위/아래 메시지를 포함하거나 추가 스크롤 후 즉시 내보낼 때는 아직 값이 어긋날 수 있으니 문제가 생기면 `전체` 버튼으로 초기화해 주세요.
