...
- **턴 범위 내보내기**: Export 섹션의 `턴 범위` 입력에 시작/끝 **플레이어 턴**을 지정하면 해당 구간만 파일로 저장할 수 있습니다. 비워두면 전체가 포함되며, `전체` 버튼으로 즉시 초기화할 수 있습니다.
...
- Export 패널의 `턴 범위` 입력을 사용하면 현재 확보된 **플레이어 턴** 중 원하는 시작/끝 지점을 선택해 부분만 내보낼 수 있습니다. 선택을 비우면 전체 대화가 포함됩니다.
