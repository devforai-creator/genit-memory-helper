...
> **🚀 v3.1.0: 메타 요약 (Meta Summary) 추가!**
> 10개 청크 요약을 1개로 통합하는 계층적 요약 기능이 추가되었습니다.
> 긴 대화도 효율적으로 유저노트에 담을 수 있습니다.
...
4. 아래 옵션들을 모두 켜기:
   - **"사용자 스크립트 허용"** ← 필수! 이게 꺼져 있으면 스크립트가 실행되지 않음
   - **"사이트 액세스"** → "모든 사이트에서" 선택
   - **"파일 URL에 대한 액세스 허용"** (있는 경우)
5. genit.ai 또는 babechat.ai로 돌아가서 새로고침 (Ctrl+Shift+R)
...
v3.0.0부터 대화를 **10개 메시지 단위 청크**로 분할하여 **요약**을 생성하고 브라우저에 영구 저장하는 기능이 추가되었습니다.

#### 주요 기능

1. **청킹**: 대화를 10개 메시지 단위로 자동 분할
2. **요약 프롬프트**: 각 청크에 대해 LLM 요약 생성용 프롬프트 복사
3. **결과 저장**: 외부 LLM에서 생성한 요약을 붙여넣기하여 IndexedDB에 영구 저장
4. **메타 요약 (v3.1.0)**: 10개 청크 요약을 1개의 통합 요약으로 압축
5. **계층적 복사**: "전체 요약 복사" 시 메타 요약 + 나머지 청크 요약으로 최적화

#### 권장 사용 방법

1. GMH 패널에서 **"🧠 Dual Memory"** 섹션 찾기
2. **"GMH에 담기"** 버튼 클릭 → 대화가 10개 메시지 단위로 분할됨
3. 각 청크에서 **"📋 요약"** 클릭 → 프롬프트 복사
4. ChatGPT/Claude 등 외부 LLM에 붙여넣기 → 결과 받기
5. 청크 펼치기 → LLM 결과 붙여넣기 → **"저장"**
6. 10개 청크 완료 시 **메타 요약 섹션**이 표시됨
7. 메타 프롬프트 복사 → LLM → 메타 요약 저장
8. **"📋 전체 요약 복사"** → 플랫폼 유저노트에 붙여넣기

#### 메타 요약 (v3.1.0)

긴 대화(100+ 메시지)의 경우, 청크 요약만으로는 유저노트 2000자 제한을 초과할 수 있습니다.
**메타 요약**은 10개 청크 요약을 ~500자의 통합 요약으로 압축합니다.

```
예시: 250개 메시지 대화
- 25개 청크 생성
- 메타 요약 2개 (청크 1~10, 11~20)
- 남은 청크 5개 (21~25)
→ "전체 요약 복사" = 메타 2개 + 청크 5개 = ~1500자
```

#### ⚠️ Facts 기능 안내

**Facts 프롬프트** 기능은 현재 구현되어 있지만, **실사용은 권장하지 않습니다**.

Facts(구체적 사실 추출)는 **의미 검색(Semantic Search)** 기능과 함께 사용해야 효과적입니다:
- Facts만 저장하면 나중에 필요한 정보를 찾기 어려움
- 향후 임베딩/RAG 기능 구현 시 Facts 검색이 가능해질 예정
- 현재는 **요약 기능만 사용**하는 것을 권장합니다

#### 유저노트 복사 옵션

- **전체 요약 복사**: 메타 요약 + 남은 청크 요약 (계층적 복사)
- **전체 Facts 복사**: 모든 청크의 Facts 복사 (권장하지 않음)
- **통합 복사**: 요약과 Facts를 함께 복사

**참고**:
- 데이터는 브라우저별로 저장되며 기기 간 동기화되지 않습니다.
- 시크릿 모드나 브라우저 초기화 시 데이터가 삭제될 수 있습니다.
- 저장된 요약은 세션별로 관리되며, 같은 대화를 다시 열면 자동으로 불러옵니다.
