...
- **턴 범위 내보내기**: Export 섹션의 `턴 범위` 입력에 시작/끝 **플레이어 턴**(가장 최근 턴부터 1, 2, … 순서) 을 적어 원하는 구간만 파일로 저장할 수 있습니다. `시작지정`/`끝지정` 버튼으로 현재 미리보기의 플레이어 턴을 바로 책갈피로 삼을 수 있으며, `전체` 버튼을 누르면 즉시 초기화됩니다.
