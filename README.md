...
- **v1.3.2**: Tampermonkey 메타헤더·패널 배지·dist 산출물의 버전을 `package.json`에서 자동으로 가져오도록 정리하고, `npm run bump:patch|minor|major`로 버전 업 + 태그 푸시까지 한번에 처리할 수 있는 릴리스 스크립트를 추가했습니다.
...
- **턴 범위 내보내기 (실험적)**: Export 섹션의 `턴 범위` 입력에 시작/끝 **플레이어 턴**(가장 최근 턴부터 1, 2, … 순서)을 적어 원하는 구간만 파일로 저장할 수 있습니다. `시작지정`/`끝지정` 버튼으로 현재 미리보기의 플레이어 턴을 책갈피로 지정할 수 있지만, 새 메시지가 로드되거나 대화 맨 위/맨 아래 메시지를 선택할 때는 아직 오차가 발생할 수 있습니다. 문제가 생기면 `전체` 버튼으로 초기화한 뒤 전체 범위를 내보내 주세요.
