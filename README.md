...
- **턴 범위 내보내기 (실험적)**: Export 섹션의 `턴 범위` 입력에 시작/끝 **플레이어 턴**(가장 최근 턴부터 1, 2, … 순서)을 적어 원하는 구간만 파일로 저장할 수 있습니다. `시작지정`/`끝지정` 버튼을 누르면 최근 클릭한 플레이어 턴이 북마크 큐(최대 5개)에 저장되고, 드롭다운에서 과거 책갈피를 다시 선택할 수 있습니다. v1.3.5에서 오프셋/누락 문제가 크게 줄었지만, 대화 최상단/최하단 메시지를 포함하거나 막 스크롤한 직후에는 드물게 범위가 빗나갈 수 있으니 이상하다 싶으면 `전체` 버튼으로 초기화해 주세요.
...
- Export 패널의 `턴 범위` 입력을 사용하면 현재 확보된 **플레이어 턴** 중 원하는 시작/끝 지점을 선택해 부분만 내보낼 수 있습니다. 턴 번호는 “가장 최근 = 1”부터 올라가므로, 오래된 턴을 지정하려면 더 큰 숫자를 입력하면 됩니다. 선택을 비우면 전체 대화가 포함되며, `시작지정`/`끝지정` 버튼으로 미리보기에서 보고 있는 턴을 바로 지정할 수 있습니다. **이 버튼은 실험 기능**이라 대화 맨 위/아래 메시지나 추가 로딩이 발생한 뒤에는 값이 어긋날 수 있으니, 문제가 느껴지면 `전체` 버튼으로 초기화하거나 직접 숫자를 입력해 주세요.
